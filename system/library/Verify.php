...
/***************************************************************************************************
 ...
 页面代码（访问控制层，获取验证码）：
 <img src="./VerifyController.php" onclick="this.src='VerifyController.php?'+Math.random();"></img>
****************************************************************************************************/
...
class Verify {
    ...
    public function check($code, $id = '') {
        ...
        $secode = json_decode($_SESSION($key),true);
    ...
    public function entry($id = '') {
        ...
        // 验证码使用随机字体
        $ttfPath = RESOURCE_PATH . '/font/Verify/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
        ...
        $_SESSION[$key.$id] = json_encode($secode);
    ...
    | @access private
    |-----------------------------------------------------------------------------------------------------  
    | @desc   高中的数学公式咋都忘了涅，写出来:正弦型函数解析式：y=Asin(ωx+φ)+b
    |         各常数值对函数图像的影响：
    |         A：决定峰值（即纵向拉伸压缩的倍数）
